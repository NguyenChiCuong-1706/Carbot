// init all basic params and stored in HW info
// init wheel params and stored in HW info
// link wheel params into HW-EXPORT interfaces (Transfer data between ros2_control and arduino)
// Give desired func for HW-WRITE : Calculate ros2_ctrl cmd (through vel_cmd topic) to motor vel (PPR), then send to arduino
// Give desired func for HW-READ : read encoder values from arduino, then cal vel
// Add target_link_libraries(diffdrive_arduino PUBLIC serial) to CmakeList.txt,   <depend>libserial-dev</depend> to pkg.xml
// Finally, add plugin and plugin params to ros2_ctrl.xacro
...
// init all params and stored in HW info
  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  // PULL OUT params in HW-INFO : 
  // stof/stoi: string to int/float convert
...
// Linking wheel params into exposed HW interfaces
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  if (cfg_.pid_p > 0)
  {
    comms_.set_pid_values(cfg_.pid_p,cfg_.pid_d,cfg_.pid_i,cfg_.pid_o); // Use comms func from arduino_comms lib
  }
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  wheel_r_.pos = wheel_l_.vel*delta_seconds;
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  // Calculate ros2_ctrl vel commnand (through vel_cmd topic) to motor vel (PPR)
